...
from util.discord_apis import delete_channel
...
class LeagueService():
    ...
    async def create_league_interaction(self, body):
    ...
    def delete_league(self, guild_id, body):
        ...
        return {
            'type': InteractionResponseType.CHANNEL_MESSAGE_WITH_SOURCE,
            'data': {
                'content': f'League `{league_name}`deleted~ {generate_random_emoji()}'
            }
        }
